...
module GoingLoopless
  ...
  def build_credits(movie, role_order)
    role_order.flat_map do |role_name|
      movie.roles
        .shuffle  # Not part of solution; helps rests remain resilient to OS-specific sorting
    ...
    # movie.roles
    #   .shuffle  # Not part of solution; helps rests remain resilient to OS-specific sorting
    #   .select { |role| role_order.include?(role.name) }
    #   .sort_by { |role| role_order.find_index(role.name) }
    #   .map { |role| "#{role.person.name} (#{role.name})" }
  end
end
